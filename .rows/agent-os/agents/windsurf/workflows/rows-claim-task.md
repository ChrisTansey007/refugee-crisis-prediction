...
4. **Write a lock file** in `agent-os/locks/` to signal intent:
   ```bash
   cat > agent-os/locks/[TASK-ID]-windsurf.json << EOF
   {
     "task_id": "[TASK-ID]",
     "claimed_by": "windsurf",
...
6. **Update task file** — set `Status` to `in-progress`, `Current Claimed Worker` to `windsurf`, and refresh `Snapshot Freshness` if you enriched it.
