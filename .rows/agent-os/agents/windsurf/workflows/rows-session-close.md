...
3. **Write a handoff file** at `agent-os/handoffs/YYYY-MM-DD-windsurf-[TASK-ID].md`:
   ```markdown
   # Handoff — [DATE]
...
4. **Update worker status** to idle:
   ...
   jq --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
     '.workers.windsurf.status = "idle" | .workers.windsurf.last_active = $ts' \
...
5. **Commit handoff**:
   ...
   git commit -m "chore(agent-os): session close — windsurf handoff"
   git push
   ```
